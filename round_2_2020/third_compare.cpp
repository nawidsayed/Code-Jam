...
// Testing personal solution (solve) against correctly judged solution from Gennady.Korotkevich (solve2).
int main(){
    long ans, ans2;
    long r1,r2;
    long max_N = 10;
    ...
    // Low mod produces test cases with high linear dependence, high mods test for integer overflow.
    for (long mod: vl({2L,5L,10L, 1000000000L})){
        ...
        FOR(i,0,iterations){
            ...
            if(ans != ans2) print("disagreement with correct solution", ans, ans2)    
        }
    }
}
