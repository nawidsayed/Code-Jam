...
long solve(long t){
    ...
    if(P == 3){
        long pairs = min(mods[1], mods[2]);
        total += pairs;
        mods[1] -= pairs;
        mods[2] -= pairs;
        long remainders = max(mods[1], mods[2]);
        total += (remainders + 2) / 3;
    }
    if(P == 4){
        long pairs22 = mods[2] / 2;
        total += pairs22;
        mods[2] -= 2*pairs22;
        long pairs13 = min(mods[1], mods[3]);
        total += pairs13;
        mods[1] -= pairs13;
        mods[3] -= pairs13;
        if(mods[1] > 1 && mods[2] > 0){
            total ++;
            mods[2] --;
            mods[1] -=2;
        }
        if(mods[3] > 1 && mods[2] > 0){
            total ++;
            mods[2] --;
            mods[3] -=2;
        }
        if (mods[2] != 0){
            ...
            long remainders = max(mods[1], mods[3]);
            total += (remainders + 3) / 4;
        }
